...
import com.denidove.Logistics.exceptions_old.RestTemplateResponseErrorHandler;
import com.denidove.Logistics.json.DelLine_old;
...
@Component
public class DellineServiceImpl_Old implements DellineService {

    private static final Logger log = LoggerFactory.getLogger(DellineServiceImpl_Old.class);
    ...
    public DellineServiceImpl_Old(RestTemplateBuilder restTemplateBuilder, ObjectMapper mapper, TaskService taskService,
    ...
    public TaskDto sendRequest(HttpServletRequest servletRequest, TaskDto taskDto) throws JsonProcessingException, HttpStatusCodeException {
        ...
        HttpEntity<DelLine_old> request = new HttpEntity<>(logistics, headers);
    ...
    private DelLine_old objectToJson(String appkey, TaskDto taskDto) {
        ...
        var deliveryType = new DelLine_old.Delivery.DeliveryType("auto");
        var address1 = new DelLine_old.Delivery.Address(startPoint);
        //var address2 = new DelLine_old.Delivery.Address("47.204150, 39.701188");
        var address2 = new DelLine_old.Delivery.Address(finishPoint);
        var time = new DelLine_old.Delivery.Time("9:30", "19:00");
        var derival = new DelLine_old.Delivery.Derival("address", CalendarUtils.getDerivalDate(), time, address1);
        //var requirements = new String[]{"0x818e8ff1eda1abc349318a478659af08"};
        var requirements = new String[]{};
        var arrival = new DelLine_old.Delivery.Arrival("address", address2, time, requirements);
        //var packages = new DelLine_old.Delivery.Packages[]{new DelLine_old.Delivery.Packages("0xA6A7BD2BF950E67F4B2CF7CC3A97C111", 1)};
        //var delivery = new DelLine_old.Delivery(deliveryType, derival, arrival, packages);
        var delivery = new DelLine_old.Delivery(deliveryType, derival, arrival);
        var paymentCitySearch = new DelLine_old.Payment.PaymentCitySearch(finishPoint);
        var payment = new DelLine_old.Payment(paymentCitySearch, "cash");

        var cargo = new DelLine_old.Cargo(1, length.floatValue(), width.floatValue(), height.floatValue(),
                Float.parseFloat(volume), Float.parseFloat(totalWeight), Float.parseFloat(oversizedWeight), Float.parseFloat(oversizedVolume));
        return new DelLine_old(appkey, delivery, payment, cargo);
    }
